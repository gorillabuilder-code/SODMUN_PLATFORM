...
const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  if (loading) return <div style={{ color: '#FF8C00', padding: '20px', fontWeight: 800 }}>Authenticating...</div>;
  return user ? <>{children}</> : <Navigate to="/login" />;
};

const ChairRoute = ({ children, role, roleLoading }: { children: React.ReactNode, role: string | null, roleLoading: boolean }) => {
  if (roleLoading) return <div style={{ color: '#FF8C00', padding: '20px', fontWeight: 800 }}>Verifying Clearance...</div>;
  
  // LOGIC FIX: Access for anyone who is NOT a Delegate
...
const AppShell = () => {
  ...
  // UI LOGIC FIX: Show Command Center for anyone who is NOT a Delegate
  ...
  return (
    <div className="app-container">
      <style>{`
        ...
        .nav-link.active { background: rgba(255,140,0,0.1); color: var(--accent); border-color: rgba(255,140,0,0.2); }
        .chair-link { margin-top: 20px; color: var(--accent); border: 1px dashed rgba(255,140,0,0.3); }
        .chair-link:hover { background: var(--accent); color: #000; border-style: solid; }
        .chair-link.active { background: var(--accent); color: #000; box-shadow: 0 0 15px rgba(255,140,0,0.3); }
      ...
      {user && (
        <aside className="sidebar">
          ...
          <nav className="nav-menu">
            ...
            {showCommandCenter && (
              <>
                <span style={{ fontSize: '11px', fontWeight: 800, color: 'var(--accent)', textTransform: 'uppercase', marginTop: '30px', marginBottom: '5px', paddingLeft: '10px' }}>
                  Chair Access
                </span>
          ...
          <div style={{ marginTop: 'auto', padding: '15px 10px', borderTop: '1px solid #1a1a1a' }}>
            <div style={{ fontSize: '11px', color: '#666', fontWeight: 700 }}>SESSION ACTIVE</div>
            <div style={{ fontSize: '12px', color: '#fff', fontWeight: 800, marginTop: '4px' }}>Secure Connection</div>
          </div>
        </aside>
      )}
